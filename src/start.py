...
def createProject(directory_path, project_name):
...
def dockerSetup(project_dir):
    with open("./templates/docker-compose.yml", "r") as ifile:
        with open(project_dir / "docker-compose.yml", "w") as ofile:
    ...
    with open("./templates/bloodhound.config.json", "r") as ifile:
        with open(project_dir / "bloodhound.config.json", "w") as ofile:
...
def startProject(args, PROJECT_DIR):
    ...
    if not createProject(Path(__file__).parent, PROJECT_DIR):
        print(Fore.RED + f'[-] The folder "{PROJECT_DIR}" could not be created.')
        print(Style.RESET_ALL + 'Exiting...')
        exit(1)
    
    # Create project directory
    createProject(PROJECT_DIR, args.project)
    
    # Setup the docker files for the project
    dockerSetup(PROJECT_DIR / args.project)
    ...
    try:
        with open("/tmp/bh-auto-log.txt", "w") as output_log:
            docker_process = subprocess.Popen(
                ["docker-compose", "up"], cwd=PROJECT_DIR / args.project, text=True, stdout=output_log, stderr=output_log
                )
